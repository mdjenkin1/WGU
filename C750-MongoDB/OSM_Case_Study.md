...
What follows is a list of scripts created to Extract, Transform, Load and Query dataset. These scripts were written and tested with Python 3.7. I have made these scripts are available at [https://github.com/mdjenkin1/WGU/tree/master/C750-MongoDB](https://github.com/mdjenkin1/WGU/tree/master/C750-MongoDB)

* [element_getter.py](.\element_getter.py): Provides a method of element iteration that serves full elements. The functionality of this script was adopted from course material on Udacity.
* [describe_xml.py](.\describe_xml.py): Metadata generator for XML data. Extracts information about the structure and content of XML data and transforms it into a JSON compatible structure.
* [xml_desc_to_mongo.py](.\xml_desc_to_mongo.py): The load portion of the ETL process for XML Metadata generation. The metadata generated by describe_xml.py will be loaded to MongoDB for further inquiry.
...
* [map_to_mongo.py](.\map_to_mongo.py): First of two scripts to handle OSM data extraction and loading to mongodb.
* [slc_street_cleanup.py](.\slc_street_cleanup.py): Leverages the extraction and loading functionality of the map_to_mongo.py script while also introducing data transformations to provide more uniform data.
...
_Note: When converting the queries from the Mongo Client to Python Script, it was necessary to encase the aggregation operators in quotes and replace the nulls with Nones._  
