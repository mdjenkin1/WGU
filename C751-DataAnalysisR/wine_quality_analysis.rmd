---
title: "Chemical Composition as Predictor of Wine Quality"
...
The datasets explored in this analysis were found [here for red](https://bit.ly/2KTzkjX) and [here for white](https://bit.ly/1QqxDBL). The accompanying documentation of the datasets can be found [here](https://bit.ly/2vinuZz).  
...
# Red wine peek at data structure 
str(wines.red)
```
...
# Summary of red wine quality scale
...
# Red wine quality pie chart data prep
red_quality_counts <- wines.red %>%
  group_by(quality) %>%
  summarise(no_obs = length(quality))

# manual color scale for this pie chart
sixColors = c('#2166ac','#67a9cf','#d1e5f0','#fddbc7','#ef8a62','#b2182b')

# Red wine quality pie chart
ggplot(data = red_quality_counts, 
       aes(x = "", y = no_obs, fill = factor(quality))) + 
  geom_bar(width = 1, stat="identity") + 
  coord_polar("y", start=0) +
  scale_fill_manual(values=sixColors, name="Quality") 
```
...
# Summary of white wine quality scale
...
# White wine quality pie chart data prep
white_quality_counts <- wines.white %>%
  group_by(quality) %>%
  summarise(no_obs = length(quality))

# manual color scale for this pie chart
sevenColors = c('#2166ac','#67a9cf','#d1e5f0','#f7f7f7',
                '#fddbc7','#ef8a62','#b2182b')

# White wine quality pie chart
ggplot(data = white_quality_counts, 
       aes(x = "", y = no_obs, fill = factor(quality))) + 
  geom_bar(width = 1, stat="identity") + 
  scale_fill_manual(values=sevenColors, name="Quality") +
  coord_polar("y", start=0)
```
...
Some rudimentary research on wine research turned up [Wine Chemistry on Wikipedia](https://en.wikipedia.org/wiki/Wine_chemistry). From this we see there's a possibly of some missing measures. For example, phenolic compounds and proteins. The possibility of missing solute data is a cause of concern. However, The accompanying documentation makes the claim no compositional information is missing. We'll proceed with the information we do have.  
...
The remaining variables are not compositional measurements. X is an index that is not linked to another dataset. It can be discarded. Quality is 
provided as an integer. There may be cause to convert it to a factor. Converting quality to a factor would reduce the scale. This may complicate any future additions to the dataset. Finally there's Ph. Ph is is a descriptor of acidity. It may be of interest to explore the relationship between total acidic composition and Ph.  
...
1. Convert density from grams per cubic centimeter to grams per cubic decimeter. (i.e. multiply by 1000)
1. Convert Sulfur dioxides from milligrams per cubic decimeter to grams per cubic decimeter. (i.e. divide by 1000)
1. Convert solute concentration values to percents. (i.e. divide their value by density and multiply by 100)
...
# Define items for the initial matrix
...
ggpairs(wines[,subjects], progress = FALSE, columnLabels = new_names,
...
# Redefine which values for the matrix
...
# Just the reds
theme_set(theme_minimal(base_size=9))
ggpairs(subset(wines[,subjects], color=="red"), 
        progress = FALSE, columnLabels = new_names,
        lower=list(combo=wrap("facethist", binwidth=15))) +
  theme(text = element_text(size=10), 
     axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size=7),
             panel.grid.minor = element_blank(), 
...
# Just the whites
theme_set(theme_minimal(base_size=9))
ggpairs(subset(wines[,subjects], color=="white"), 
        progress = FALSE,  columnLabels = new_names,
        lower=list(combo=wrap("facethist", binwidth=15))) +
  theme(text = element_text(size=10), 
     axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size=7),
...
# Scatterplot of alcohol content at levels of quality. Crowded
...
# Scatterplot of alcohol content at levels of quality. Less crowded
ggplot(data=wines, aes(x=quality, y=alcohol)) +
  geom_jitter()
```
...
# Scatterplot of alcohol content at levels of quality. Least crowded
ggplot(data=wines, aes(x=quality, y=alcohol)) +
  geom_jitter(alpha=.4) 
```
...
# Scatterplot of alcohol content in clasifications of quality
...
# Scatterplot of alcohol content with binned by quality
wines %>%
...
# Scatterplot of alcohol content in best quality wines
wines %>% 
...
# Scatterplot of alcohol content in average quality wines
wines %>% 
...
# Scatterplot of alcohol content in least quality wines
wines %>% 
  subset(as.numeric(quality) <= 3) %>%
  ggplot(aes(x=quality, y=alcohol)) +
  geom_jitter(alpha=.4)
...
# Mixed graph of alcohol content at all wine quality
# Missing the middle range of alcohol content
wines %>% 
...
# Scatterplots of alcohol at binned qualities
# Missing quality of ambiguous value
wines %>%
...
# unadjusted correlation, alcohol x quality 
...
# correlation, alcohol x quality
# minus ambiguous quality
...
# Alcohol Summary for granularity of range
...
# Salt x alcohol scatter, adjusted
# with moving average
...
# Sugar x alcohol scatter, adjusted
# with moving average
...
# Sugar x alcohol scatter, adjusted
# with moving average by varietal
ggplot(data=subset(wines, residual.sugar <= 2.5), 
       aes(x=alcohol, y=residual.sugar)) + 
  scale_y_continuous() +
  geom_jitter(alpha=.4) +
...
# comparison of red wine sugar content 
# at alcohol level and at quality level
...
# pivoting for graphing alcohol content at levels of salt and sugar
wine.ratios <- melt(wine.tastes, id.vars = c("sugar", "salt", "type"))
```

```{r echo=FALSE, message=FALSE, warning=FALSE, Beginning_of_the_End}
# Initial heat map of alcohol at salt x sugar - 2 colors
...
# heat map of alcohol at salt x sugar - 3 colors
ggplot(aes(y = sugar, x = salt, fill = value),
  data = wine.ratios) +
  geom_tile() +
...
# heat map of alcohol at salt x sugar - 4 colors
ggplot(aes(y = sugar, x = salt, fill = value),
  data = wine.ratios) +
  geom_tile() +
...
# Zooming in on the happy spot
# Where alcohol is the highest
ggplot(aes(y = sugar, x = salt, fill = value),
  data = wine.ratios) +
  geom_tile() +
...
# Adjusting the colors in the happy spot zoom
ggplot(aes(y = sugar, x = salt, fill = value),
  data = wine.ratios) +
  geom_tile() +
...
# Zooming back out with new color scheme
ggplot(aes(y = sugar, x = salt, fill = value),
  data = wine.ratios) +
  geom_tile() +
  scale_fill_gradient2(low = "white", mid = "blue", high = "black", 
...
# Alcohol heat graph at levels of salt x sugar
# Just the Reds
ggplot(aes(y = sugar, x = salt, fill = value),
...
# Consistent theme for final formating
...
# Alcohol as Quality
# Boxplot overlayed on Scatterplot
wines %>% 
  subset(alcohol <= 14.5) %>%
  ggplot(aes(x=quality, y=alcohol)) +
...
# Scatterplot of salt content at levels of alcohol
# Differences and simularities of varietals added
# Converted salts from percent to PPM (x10000)
...
# Heat map of alcohol at various levels of salt x sugar
# Moving Average of Varietals imposed
# Converted salts and sugars to milligrams and grams (x1480, x1.48)
