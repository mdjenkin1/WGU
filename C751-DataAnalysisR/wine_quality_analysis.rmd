...
```{r echo=FALSE, Salt_and_Alcohol_Refined}
...
```{r echo=FALSE, Sugar_and_Alcohol}
...
```{r echo=FALSE, Sugar_and_Alcohol_Refined}
...
```{r echo=FALSE, Sugar_and_Alcohol_by_Varietal}
...
```{r echo=FALSE, Salt_and_Alcohol_by_Varietal}
ggplot(data=subset(wines, chlorides < .03), aes(x=alcohol, y=chlorides)) +
  geom_jitter(alpha=.4) +
...
```{r echo=FALSE, Sugar_and_Alcohol_in_Reds}
...
```{r echo=FALSE, Sugar_Quality_Reds}
ggplot(data=subset(wines, residual.sugar <= 0.7 & color == "red"), 
...
```{r echo=FALSE, Red_Sugar_Quality_vs_Alcohol}
...
```{r echo=FALSE, Data_Melt}
wine.ratios <- melt(wine.tastes, id.vars = c("sugar", "salt", "type"))
```

```{r echo=FALSE, Beginning_of_the_End}
...
better see where there's not enough sugar for the amount of salt in that bright \
blue. Just above the bright blue, we see a distribution of higher alcohol \
content wines extending into higher sugar content.  
...
```{r echo=FALSE, Reds_Salt_and_Sugar}
ggplot(aes(y = sugar, x = salt, fill = value),
...
```{r echo=FALSE, Whites_Salt_and_Sugar}
ggplot(aes(y = sugar, x = salt, fill = value),
...
```{r echo=FALSE, Building_the_Final_Plot_Theme}
...
```{r echo=FALSE, Alcohol_As_Quality}
wines %>% 
  subset(alcohol <= 14.5) %>%
  ggplot(aes(x=quality, y=alcohol)) +
