...
Percents are commonly understood.
...
One final graph I want to look at is a heat map of alcohol content at ratios \
of sugar to alcohol. First I need to understand the melt function.

```{R}
wine.select <- c("alcohol","residual.sugar","chlorides")
wine.tastes <- wines[wine.select]
head(wine.tates)
...
```{R}
melt(wine.ratios, id.vars = c("residual.sugar","chlorides"))
```

```{R}
ggplot(aes(y = residual.sugar, x = chlorides, fill = alcohol),
  data = wine.ratios) +
  geom_tile() +
