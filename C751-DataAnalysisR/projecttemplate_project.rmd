...
ggplot(data=subset(wines, chlorides < .03), aes(x=alcohol, y=(chlorides*100))) +
  geom_jitter(alpha=.3, shape = 21, fill = I('#e2c55a')) +
  scale_y_log10(limits=c(0.15, 1.75)) +
  geom_smooth(aes(color=color)) +
  labs(title="Quality and Salt Content Across Varitals", y="Milligrams of Salt",
       x = "Percent Alcohol Content") +
  theme(plot.title = element_text(size = 16, face="bold"), 
        axis.title.x = element_text(size = 10, face="bold"),
        axis.title.y = element_text(size = 10, face="bold"),
        panel.background = element_rect(fill = "lightgrey", color = "darkgrey"),
...
Using alcohol content as a direct measurement to quality revealed salt content \
as a major factor for determing wine quality. Across both varities we see that \
as alcohol content increases, salt content decreases. 

The amount of salt in a sample of wine is miniscule compared to the amount of \
alcohol. Where we see alcohol in measures of 8-14% by volume, we see salt in \
measures of thousandths of a percent. It is interesting that such a small \
variation in one compound can have such a dramatic effect on quality.
