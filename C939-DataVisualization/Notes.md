...
## Finally Loading data

After investigating the code I left for myself, I set to building the final load script. Instead of focusing on data issues while loading, I instead decided to generate metadata outlining the potential issues.  

On loading the data some new challenges needed to be addressed.  
First, there was the issue shear data size. This was handled by introducing batch processing.  
To avoid reprocessing, data offsets were used to resume following any additional errors.  

```{Mongodb}
> db.raw_schedules.count()
123534969
> db.detailed_flights.count()
89742593
>
```

One script anamoly encountered is the timezone data differences between timezonefinder and pytz.
Timezones in the airport definitions collection were generated by timezonefinder.
...
db = mdb_client['air_travel']
tzf_zones = db.airports.distinct("timezone")

for zone in tzf_zones:
   if zone not in pytz.all_timezones:
      print("{} is not in pytz".format(zone))

>>>
America/Central is not in pytz
>>>

for airport in central_airports:
  print("Central time airport: {}".format(airport))

>>>
Central time airport: {'_id': ObjectId('5eb60b225debda56d124a728'), 'iata': 'CBM', 'airport': 'Columbus Air Force Base', 'city': 'Columbus', 'state': 'MS', 'country': 'USA', 'lat': 33.64379883, 'long': -88.44380188, 'timezone': 'America/Central'}
>>>

print("US/Central in pytz") if "US/Central" in pytz.all_timezones else print("US/Central NOT in pytz. Find another")

>>>
US/Central is in pytz
>>>

db['airports'].update_many({"timezone": "America/Central"}, {'$set': {"timezone": "US/Central"}})
```

At the end of that hunt. It turns out the America/Central airport was the airport manually added in script. So a cleaner solution of updating the source csv with the missing entry and removing the in script manual add was performed.

```{mongodb}
db.airports.find({iata:"CBM"})
{ "_id" : ObjectId("5eb6272ad663a8533f0dbc63"), "iata" : "CBM", "airport" : "Columbus Air Force Base", "city" : "Columbus", "state" : "MS", "country" : "USA", "lat" : 33.64379883, "long" : -88.44380188, "timezone" : "America/Chicago" }
```
...
After a day long run, the dataset has loaded successfully to Mongodb. On closer inspection, the full carrier name appears to be missing from the final load. Running an update process in parallel with Tableau processing is possible.

## On to Tableau

With structured data finally loaded to MongoDB, the only prep that remains is setting up an ODBC connector that can be used by Tableau. This was easier than one might expect. A little bit of configuration and the full dataset is now available for investigation.  

Unfortunetly the performance is untenable. A revisiting of the MongoDB schema may be in order. Starting with the information here: [https://www.mongodb.com/blog/post/6-rules-of-thumb-for-mongodb-schema-design-part-1](https://www.mongodb.com/blog/post/6-rules-of-thumb-for-mongodb-schema-design-part-1)  

Another option may be to extract the data for a specific question.  
e.g. How has traffic for each airport in our dataset evolved over time?  

To help with the performance issue, I've added a sql query to limit the fields retrieved from MongoDB.  
